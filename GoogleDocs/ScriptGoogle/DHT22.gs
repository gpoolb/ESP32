...
function doGet(e) {
  ...
  try {
    ...
    if (tag == "download"){
      ...
      //return downloadFile(nameFile); // Este metodo requiere el uso de convertRangeToCsvFile_() 
	  var ssLogging = SpreadsheetApp.openById(searchFile(nameFile));
      var exportUrl = 'https://docs.google.com/spreadsheets/d/' + ssLogging.getId()
     ...
     //Hacemos la petición para pedir el fichero exportado
      ...
      output.downloadAsFile(nameFile + ".csv");
      return output;
...
//  Logger.log("--- save_data ---");
  try {
    ...
    Logger.log("Falló al guardar documento..." + error.message);
  }
...
//  Logger.log("--- read_data ---");
  try {
    ...
    Logger.log("Falló al leer documento..." + error.message);
  }
...
function loadOptionValues() {
  ...
  while (j < 14){
    ...
    //Logger.log("\n--- Archivo Actual = : " + tagNameMonth [i] + anio.toString());
    try {
      // Se verifica el archivo que corresponde a cada mes del año en los históricos
      if (searchFile(tagNameMonth [i] + anio.toString()) != "-1") {
        //Logger.log(" ... encontrado\n");
        xmlContent += "<secciones>" + j.toString() +"</secciones>\n"; 
      }

    } catch (e) {
	    Logger.log(" ... liberando\n" + e.message);
      liberaArchivo(tagNameMonth [i] + anio.toString());  // Metodo pendiente por evaluar
	  }
...
//  Logger.log("--- createNewFiles --- init: " + searchFolder("Respaldo"));
  ...
  if( IdFile == -1) {
    ...
    try {
      ...
      Logger.log('Could not obtain lock after 30 seconds.' + error.message);
      return HtmlService.createHtmlOutput("<b> Server Busy please try after some time <p>")
   ...
   try
      {
        ...
        return output;
           ...
           Logger.log("--- downloadFile --- " + e.message );
           return ContentService.createTextOutput('Nothing To Download' + e.message)
      } 
}
...
/*
    Por políticas de Google los archivos pueden bloquearse
    si no presentan actividad antes de 1 Julio 2021, este
    metodo escribe la hora en una celda sin utilizar para que
    se actualice la fecha de modificación (ultimo acceso)
*/
function liberaArchivo(nameFile) {

  var flag = -1;
  //Logger.log("---LiberaArchivo --- ");

  try {

    var ssLogging = SpreadsheetApp.openById(searchFile(nameFile));  // Abre la instancia del archivo que guarda los históricos
    var dataLoggerSheet = ssLogging.getSheetByName("Hoja 1");  // Abre la instancia de la hoja 1 en la hoja de calculo para guardar los históricos

	  // Se bloquea el acceso a éste método para evitar
	  // que este metodo se ejecute varias veces al mismo tiempo (Multitarea)
	  var lock = LockService.getScriptLock();
	  try {
	    lock.waitLock(3000); // wait 3 seconds for others' use of the code section and lock to stop and then proceed
	  } catch (e) {
	    Logger.log('Could not obtain lock after 30 seconds.' + e.message);
	    return flag
	  }
	  // Se modifica el archivo para liberarlo de la política de Google (Sin acceso a archivos antiguos < 1 Jun 2021)
    dataLoggerSheet.getRange("F1").setValue(Utilities.formatDate(new Date(),"GMT", "''HH:mm:ss")); // Time
    SpreadsheetApp.flush();
    ...
    Logger.log("Falló al modificar documento..." + error.message);
    return flag;
  }
